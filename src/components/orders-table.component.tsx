...
const OrdersTable = ({ orders }) => {
  ...
  const saveSubdomain = async (orderId, subdomain) => {
    ...
    try {
      ...
      // Update the subdomain state to match the saved value
      setSubdomains((prev) => ({
        ...prev,
        [orderId]: subdomain,
      }));
  ...
  const handleProcessAndUploadImages = async (order) => {
    ...
    try {
      ...
      for (const [index, { filename, resizedBuffer }] of resizedImages.entries()) {
        // Convert resizedBuffer to a Blob
        const blob = new Blob(
          [Uint8Array.from(atob(resizedBuffer), (c) => c.charCodeAt(0))],
          { type: "image/jpeg" }
        );
  
        // Request signature and other details from the server
        ...
        formData.append("file", blob, filename); // Append Blob here
        ...
        setLoadingOrders((prev) => ({
          ...prev,
          [`progress_${order.id}`]: (uploadedCount / resizedImages.length) * 100,
        }));
      }
  
      // Save the URLs to the `story-photos` metafield
  ...
  const handleCopyProperties = (order, properties) => {
    ...
    const textToCopy = `story id: ${storyId}\n` +
                ...
                <div className="col-span-6 p-4 text-gray-800 dark:text-gray-300">
                  ...
                  {toggledRows[order.id] ? ( // Check toggled state
                    ...
                    order.line_items[0].properties
                      ...
                      .map((prop) => (
                        <div key={prop.name}>{prop.value}</div>
                      ))
                  )}
                </div>
...
{/* Upload Images Button */}
<button
  className={`relative p-1 pt-2 pr-2 pl-2 bg-gray-700 hover:bg-gray-900 ${
    loadingOrders[order.id] ? "text-gray-500 cursor-not-allowed" : "text-green-500 hover:text-green-600"
  } transition`}
  onClick={(e) => {
    e.stopPropagation();
...
export async function getServerSideProps() {
  const mockOrders = [
    {
      ...
      line_items: [
        {
          properties: [
            { name: "title", value: "Custom T-Shirt" },
            { name: "image", value: "https://via.placeholder.com/150.png" },
            { name: "image", value: "https://via.placeholder.com/200.png" },
          ],
        },
      ],
    },
  ...
  return { props: { orders: mockOrders } };
}
